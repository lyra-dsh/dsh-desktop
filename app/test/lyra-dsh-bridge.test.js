...
const host = require('@omnilyra/lyra-dsh-bridge')

test('lyra-dsh-bridge exports a plugin with name and apply', () => {
  assert.strictEqual(host.name, 'lyra-dsh-bridge')
...
test('createTransportRuntime proxies void methods and subscribe', async () => {
  ...
  assert.deepStrictEqual(calls[2], ['addTrayItem', [{ id: 'x' }]])
