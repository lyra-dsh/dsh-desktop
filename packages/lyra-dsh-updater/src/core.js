...
function createUpdater({ targets = [], autoDownload = true } = {}) {
  ...
  function emit(patch) {
    ...
    for (const fn of [...listeners]) {
      try { fn(status) } catch (cause) { console.error('[lyra-dsh-updater] listener error:', cause) }
    }
    return status
  }
  ...
  async function check() {
    ...
    for (const target of targets) {
      try {
        ...
        console.error(`[lyra-dsh-updater] check error on "${target.id}":`, cause)
      }
    }
    if (available) {
  ...
  function install() {
    ...
    try {
      ...
      console.error('[lyra-dsh-updater] install error:', cause)
    }
  }
