...
/**
 * lyra-dsh-keep-awake：任何会话（含子 agent 会话）在运行时，阻止系统休眠（断网），
 * 但允许息屏。
 *
 * 只负责「电源」这一件事；系统通知见 lyra-dsh-notifications，托盘状态点见
 * lyra-dsh-badge。只注入 `desktopRuntime`，事件走 cordis 的 `ctx.on`。
 */
...
function keepAwake(ctx, enabled) {
  try {
    ...
    console.error('[lyra-dsh-keep-awake] setKeepAwake failed:', cause)
  }
}

module.exports = {
  name: 'lyra-dsh-keep-awake',
