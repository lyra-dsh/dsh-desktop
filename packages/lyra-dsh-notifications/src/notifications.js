...
/**
 * lyra-dsh-notifications：在 dsh 内订阅会话/审批/提问/出错事件，弹系统通知。
 *
 * 只负责「系统通知」这一件事；托盘状态点见 lyra-dsh-badge，防休眠见
 * lyra-dsh-keep-awake。只注入 `desktopRuntime`，事件走 cordis 的 `ctx.on`。
 */
...
function notify(ctx, title, body, sound) {
  try {
    ...
    console.error('[lyra-dsh-notifications] notify failed:', cause)
  }
}
